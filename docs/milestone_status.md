# Project Milestone Status

> **Current milestone: M25 — Death, Progression & Game Over**
> Death saves, TPK game-over, XP/leveling, resurrection & training.

## Phase 1: The Core Foundation
- [x] **M1: Basic Grid Map & React Setup**
- [x] **M2: Movement Logic & Input Handling**
- [x] **M3: Basic 3D Rendering (Raycaster/React-Three-Fiber)**
- [x] **M4: Wall Collisions & Movement Boundaries**
- [x] **M5: Basic UI Layout (Zustand integration)**
- [x] **M6: Doors & Interactivity**
- [x] **M7: Combat Foundations**
- [x] **M8: Turn-based Combat Loop**
- [x] **M9: Map Items & Inventory System**
- [x] **M10: Game Over & Win Conditions**
- [x] **M11: Fog of War & Scene Atmosphere**
- [x] **M12: Saving, Loading & UI Polish**

## Phase 2: Building Actual Dungeons
- [x] **M13: Multi-Level Architecture**
- [x] **M14: JSON Dungeon Format & Loader**
- [x] **M15: In-Browser Map Builder**

## Phase 3: Gameplay & Advanced Systems
- [x] **M16: Advanced Combat & Magic System** (MP, spells, status effects)
- [x] **M17: Interactive Puzzles & Traps**
- [x] **M18: The Automap**
- [x] **M19: NPCs, Dialogue, & Shops**
- [x] **M20: Dungeon Modules (ZIP Imports)**

## Phase 4: Atmosphere & Polish
- [x] **M21: Lighting, Torches & Resting**
- [x] **M22: Ranged Combat & Targeting**
- [x] **M23: Sound, Music & Atmosphere**
- [x] **M24: Character Creation & Party Management**
- [ ] **M25: Death, Progression & Game Over**
